  ...
  <PageLayout :breadcrumbs="breadcrumbs">
    ...
    <div class="analytics-content">
      ...
      >
        ...
        <template v-if="includeQrCodeColumn" #body-cell-qrCodeName="scope">
          <q-td :props="scope">
            <router-link
              v-if="scope.row.qrCodeId"
              :to="`/qr-codes/${scope.row.qrCodeId}/analytics`"
              class="text-primary text-weight-medium"
            >
              {{ scope.row.qrCodeName || '-' }}
            </router-link>
            <span v-else>{{ scope.row.qrCodeName || '-' }}</span>
          </q-td>
        </template>
      </q-table>
    </div>
...
type ScanAnalyticsQueryParams = Pick<ScanRecordsQueryParams, 'QRCodeId' | 'StartDate' | 'EndDate'>;

const props = defineProps<{
  qrCodeId?: string;
  breadcrumbs: Breadcrumb[];
  includeQrCodeColumn?: boolean;
}>();

const { t, locale } = useI18n();

const includeQrCodeColumn = computed(() => props.includeQrCodeColumn ?? false);
const currentQrCodeId = ref<string | null>(props.qrCodeId ?? null);
...
const columns = computed<QTableProps['columns']>(() => {
  ...
  if (includeQrCodeColumn.value) {
    baseColumns.unshift({
      name: 'qrCodeName',
      label: t('analytics.qr_code_name'),
      field: 'qrCodeName',
      align: 'left',
      sortable: true,
    });
...
async function fetchScanRecords(paginationTable: PaginationTable) {
  ...
  if (currentQrCodeId.value) {
    query.QRCodeId = currentQrCodeId.value;
  }
...
async function fetchScanAnalytics() {
  const query: ScanAnalyticsQueryParams = {
    StartDate: dateRangeIso.value.start,
    EndDate: dateRangeIso.value.end,
  };
...
watch(
  () => props.qrCodeId,
  (newId) => {
    currentQrCodeId.value = newId ?? null;
